...
def payment_done(request):
    ...
    if payment.status == "paid":
        ...
        cart = Cart.objects.filter(user=user)
        for c in cart:
            # Set status based on payment success or failure
            if payment.status == "paid":
                status = "completed"  # Order is complete if payment is successful
            else:
                status = "failed"  # You can set this for failed payments

            # Save the order with the real payment status
            OrderPlaced(
                user=user,
                customer=customer,
                product=c.product,
                quantity=c.quantity,
                payment=payment,
                status=status  # Set real payment status here
            ).save()
            c.delete()
        ...
        # Handle case where payment wasn't successful
